...
from devproject.api import config, configs, explore, get_active_config
...
def dev() -> None:
    ...
    parser.add_argument("cmd", choices=["config", "configs", "explore"])
    ...
    if args.cmd == "config":
        ...
        parser = ArgumentParser(formatter_class=_formater_class)
        parser.add_argument(
            "directory", type=Path, help="directory to explore"
        )
        parser.add_argument(
            "--config",
            type=str,
